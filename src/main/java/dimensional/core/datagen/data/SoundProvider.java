package dimensional.core.datagen.data;
...
public class SoundProvider extends SoundDefinitionsProvider implements IDataProvider {
    ...
    public SoundProvider (PackOutput output, ExistingFileHelper helper) {
        super(output, DimensionalCore.MODID, helper);
    }

    @Override
    public @NotNull String getName () {
        return DimensionalCore.NAME + " Sounds";
    }

    @Override
    public void registerSounds () {
        add(CoreSounds.PLASTIC_BLOCK_PLACE, SoundDefinition.definition()
                .with(sound(DimensionalCore.MODID + ":plastic_block_place", SoundDefinition.SoundType.SOUND)
                        .volume(1.f).pitch(1.f).weight(1).stream(true))
                .subtitle("subtitles.dimensionalcore.plastic_block_place"));

        add(CoreSounds.PLASTIC_BLOCK_BREAK, SoundDefinition.definition()
                .with(sound(DimensionalCore.MODID + ":plastic_block_break", SoundDefinition.SoundType.SOUND)
                        .volume(1.f).pitch(1.f).weight(1).stream(true))
                .subtitle("subtitles.dimensionalcore.plastic_block_break"));
    }
