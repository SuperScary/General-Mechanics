...
public class CoreItemModelProvider extends ItemModelProvider implements IDataProvider {
    ...
    public ItemModelBuilder rawPlasticItem (ItemDefinition<?> item) {
        return this.getBuilder(item.id().getPath()).parent(new ModelFile.UncheckedModelFile("item/handheld")).texture("layer0", DimensionalCore.getResource("item/raw_plastic"));
    }

    public ItemModelBuilder plasticItem (ItemDefinition<?> item) {
        return this.getBuilder(item.id().getPath()).parent(new ModelFile.UncheckedModelFile("item/handheld")).texture("layer0", DimensionalCore.getResource("item/plastic"));
    }
