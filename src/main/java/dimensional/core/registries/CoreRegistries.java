...
public class CoreRegistries {
    ...
    private static <T> Registry<T> create (ResourceKey<? extends Registry<T>> key) {
        var reg = new RegistryBuilder<>(key).sync(true).defaultKey(DimensionalCore.getResource("empty")).create();
        REGISTRIES.add(reg);
        return reg;
    }

    private static <T> ResourceKey<Registry<T>> createRegistryKey (String name) {
        return ResourceKey.createRegistryKey(DimensionalCore.getResource(name));
    }
